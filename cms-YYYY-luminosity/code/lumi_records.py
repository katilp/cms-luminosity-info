...
# NB in the  +var+ notation, var needs to be a string
    rec["abstract"]["description"] = (
            ...
            + "<p>The integrated luminosity for validated runs and luminosity sections of the %s taken in %s (%s) is available in <a href=\"/record/%s/files/%slumi.txt\">%slumi.txt</a>. %s</p>" % (collision_text, year, ",".join(od_runs), ",".join(od_runs), recid, pp_text)
